...
public class Quiz extends JFrame implements ActionListener{
	...
	Quiz(String name){
		...
		ImageIcon i = new ImageIcon(ClassLoader.getSystemResource("images/quiz1.jpg"));
		...
		qno.setFont(new Font("tohoma",Font.PLAIN,24));
		add(qno);
		
		ques = new JLabel();
		ques.setBounds(150,450,900,30);
		ques.setFont(new Font("tohoma",Font.PLAIN,24));
		add(ques);
	...
	public void paint(Graphics g) {
		...
		if(ans_given ==1 ) {
			...
			if(count==9) {
				...
				for(int i=0; i<userAnswers.length; i++) {
					if(userAnswers[i][0].equals(userAnswers[i][1])) {
						score +=10;
					}else {
						score +=0;
					}
